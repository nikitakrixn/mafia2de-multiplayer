...
pub struct CCarVTable {
    ...
    /// [75] `IsModelRendered()`.
    pub is_model_rendered: usize,
    /// [76] `IsInOptimPlayerRange()`.
    pub is_in_optim_player_range: usize,
    /// [77] `IsVehicleInOpenSpace()`.
    pub is_vehicle_in_open_space: usize,
    /// [78] Заглушка.
    pub _slot_78: usize,
    /// [79] `OnBeforeLoad()`.
    ...
    pub set_vehicle_dirty: usize,
    /// [87] `SetSPZText(char const*, bool)`. Номерной знак.
    pub set_spz_text: usize,
    /// [88] `GetSPZText()`.
    pub get_spz_text: usize,
...
const _: () = {
    ...
    assert!(std::mem::offset_of!(CCarVTable, camera_pos) == 73 * 8);
