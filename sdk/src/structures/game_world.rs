...
pub struct GameManager {
    ...
    ///
    /// Это основной глобальный timestamp игры.
    /// Инкрементируется в `C_Game::Tick`.
    ///
    /// Использование по коду:
    /// - сохранение снимка (`saved_tick = current_tick`)
    /// - вычисление дельты (`current_tick - saved_tick`)
    /// - расчёт дедлайнов (`current_tick + N`)
    /// - проверки "в этом же тике или нет"
