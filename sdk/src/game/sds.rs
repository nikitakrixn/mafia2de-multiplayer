...
/// Загрузить SDS линию через native path.
pub fn activate_stream_map_line(name: &str) -> bool {
    ...
    let Some(sds_mgr) = sds_mgr else {
        logger::error("[sds] SDSManager не готов");
        return false;
    };
    ...
    if ret.is_null() || result == 0 {
        logger::warn(&format!("[sds] '{}' — не найдена или ошибка загрузки", name));
        false
    } else {
        logger::info(&format!("[sds] '{}' — загружено", name));
        true
    }
}
...
pub fn find_line_index(name: &str) -> Option<u32> {
    ...
    let cache_begin = unsafe { memory::read_ptr_raw(mgr2 + 0x18)? };
    let cache_end = unsafe { memory::read_ptr_raw(mgr2 + 0x20)? };
    ...
    while lo < hi {
        let mid = lo + (hi - lo) / 2;
        let h = unsafe { memory::read_value::<u64>(cache_begin + mid * 24).unwrap_or(0) };
        if h < target_hash { lo = mid + 1; } else { hi = mid; }
    }

    if lo < count {
        let found = unsafe { memory::read_value::<u64>(cache_begin + lo * 24).unwrap_or(0) };
        if found == target_hash {
            return unsafe { memory::read_value::<u32>(cache_begin + lo * 24 + 20) };
        }
    }
    None
}
